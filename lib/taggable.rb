    ...
    module Taggable #:nodoc:
      ...
      module SingletonMethods
        ...
        def find_tagged_with(options = {}) 
          ...
          sql = "select posts.* from posts inner join tags_posts on tags_posts.post_id = posts.id inner join tags on tags.id = tags_posts.tag_id where tags.name = '#{tag_names.join("' OR #{t}.name='")}'"
          sql << " and #{sanitize_sql(options[:conditions])} " if options[:conditions]
          sql << " order by #{options[:order]} " if options[:order]
        ...
        def tags_count(options = {})
          ...
          if options[:current_only]
            no_future = " and posts.created_at <= '#{Time.sl_local_db}'"
          else
            no_future = ''
          end
          
          sql = "select tags.id as id, tags.name as name, count(*) as count from tags_posts, posts, tags where tags_posts.tag_id = tags.id and tags_posts.post_id = posts.id and #{sanitize_sql('posts.is_active = true')}#{no_future} group by tags.name, tags.id"
          sql << " having count(*) #{options[:count]}" if options[:count]
          sql << " order by #{options[:order]}" if options[:order]
          sql << " limit #{options[:limit]}" if options[:limit]
          sql << " offset #{options[:offset]}" if options[:offset]
